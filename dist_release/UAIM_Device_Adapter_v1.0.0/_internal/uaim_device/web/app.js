...
function handleIncomingEvent(evt) {
  ...
  // Update CipherLab RS38 Wedge UI if present
  const wedgeCtrl = window.RS38_WEDGE || window.rs38Wedge;
  if (wedgeCtrl && (evt.device_id === "HH-001" || evt.source === "HANDHELD" || evt.source === "RFID" || evt.source === "QR")) {
    wedgeCtrl.handleExternalEvent(evt);
  }
...
// Select Active Device for Settings & Commands
function selectDevice(devId) {
  ...
  document.getElementById("cfg-dedup").value = dev.deduplication?.window_ms || 500;

  // Toggle Mode Sections
  if (dev.device_type === "RFID_FIXED") {
    document.getElementById("mode-section-fixed").style.display = "block";
    document.getElementById("mode-section-handheld").style.display = "none";
    if (dev.fixed_mode === "EVENT_BASED") {
      document.getElementById("mode-eventbased").checked = true;
    } else {
      document.getElementById("mode-autoscan").checked = true;
    }
  } else {
    document.getElementById("mode-section-fixed").style.display = "none";
    document.getElementById("mode-section-handheld").style.display = "block";
    document.getElementById("cfg-handheld-mode").value = dev.handheld_mode || "KEYSTROKE_EMULATION";
  }

  // Populate Formatting fields
  const fmt = dev.formatting || {};
  document.getElementById("cfg-prefix").value = fmt.prefix || "";
  document.getElementById("cfg-suffix").value = fmt.suffix || "";
  document.getElementById("cfg-strip-prefix").value = fmt.strip_prefix || "";
  document.getElementById("cfg-epc-mask").value = fmt.epc_filter_prefix || "";
  document.getElementById("cfg-min-rssi").value = fmt.min_rssi_dbm !== null && fmt.min_rssi_dbm !== undefined ? fmt.min_rssi_dbm : "";
  document.getElementById("cfg-format-mode").value = fmt.format_mode || "HEX_CLEAN";

  updateFormattingPreview();
}

// Live Formatting Preview Calculator
function updateFormattingPreview() {
  const sample = "E20034120123456789ABCDEF";
  const prefix = document.getElementById("cfg-prefix")?.value || "";
  const suffix = document.getElementById("cfg-suffix")?.value || "";
  const strip = document.getElementById("cfg-strip-prefix")?.value || "";
  const mode = document.getElementById("cfg-format-mode")?.value || "HEX_CLEAN";
  ...
  const box = document.getElementById("formatting-preview-box");
  if (box) {
...
async function saveDeviceConfig(e) {
  ...
  const dev = devicesCache.find(d => d.device_id === devId);
  const isFixed = dev && dev.device_type === "RFID_FIXED";

  const fixedMode = document.querySelector('input[name="fixed_mode"]:checked')?.value || "AUTOSCAN";
  const handheldMode = document.getElementById("cfg-handheld-mode")?.value || "KEYSTROKE_EMULATION";
  const minRssiVal = document.getElementById("cfg-min-rssi").value;

  const payload = {
    host: document.getElementById("cfg-host").value,
    port: parseInt(document.getElementById("cfg-port").value) || null,
    station_id: document.getElementById("cfg-station").value,
    deduplication_window_ms: parseInt(document.getElementById("cfg-dedup").value) || 500,
    fixed_mode: isFixed ? fixedMode : null,
    handheld_mode: !isFixed ? handheldMode : null,
    prefix: document.getElementById("cfg-prefix").value,
    suffix: document.getElementById("cfg-suffix").value,
    strip_prefix: document.getElementById("cfg-strip-prefix").value,
    epc_filter_prefix: document.getElementById("cfg-epc-mask").value,
    min_rssi_dbm: minRssiVal !== "" ? parseFloat(minRssiVal) : null,
    format_mode: document.getElementById("cfg-format-mode").value,
  };
...
function initGlobalScannerListener() {
  // Global capture for USB / Bluetooth gun scanners
  let buffer = "";
  let lastKeyTime = Date.now();

  document.addEventListener("keydown", (e) => {
    // If user is focused on a text input, don't hijack unless it's the wedge input
    if (document.activeElement && document.activeElement.tagName === "INPUT" && document.activeElement.id !== "wedge-input") {
      return;
    }
    ...
    if (e.key === "Enter" && buffer.length > 2) {
      e.preventDefault();
      document.getElementById("wedge-input").value = buffer;
      submitWedgeScan();
      buffer = "";
